...
def __put_texts_on_image(image, curvature, center_distance):
...
def pipeline(img, mtx=None, dist=None, fname=None, testMode=False):
	...
	### Step 5
	ploty, left_fitx, right_fitx, curvature, center_distance, identified_lanes = find_lanes.find_lanes_with_histogram(warped)
	filled_image = find_lanes.fillPolySpace(identified_lanes, left_fitx, right_fitx, ploty)

	### Step 6
	rewarped = perspective_transform.warp(identified_lanes, toBirdView=False)
	result = perspective_transform.weighted_img(dst, rewarped, α=0.8, β=1, λ=0)

	### Step 7
	result = __put_texts_on_image(result, curvature, center_distance)
...
def test_pipeline(images):
	...
	for fname in images:
		...
		pipeline(img, mtx, dist, fname, testMode=True)

def video_pipeline(image):
	global mtx, dist
	return pipeline(image, mtx, dist, testMode=False)
